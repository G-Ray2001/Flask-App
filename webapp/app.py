...
# Set up logging with Flask's logger
app.logger.setLevel(logging.DEBUG)
handler = logging.FileHandler('/home/pi/webapp/logs/webui.log')
...
def initialize_recording_status():
    try:
        ...
        status = {"recording": False, "current_file": None}

        # ✅ Overwrite the file to ensure it's correctly set
        with open(RECORDING_STATUS_PATH, 'w') as f:
            json.dump(status, f)

        print("✅ Recording status initialized:", status)
...
def restart_pipeline():
    global pipeline
    print("Restarting GStreamer pipeline...")

    try:
        stop_pipeline()  # ✅ Stop current pipeline
        start_pipeline()  # ✅ Start with new device selection
        return "Pipeline restarted successfully.", 200
    except Exception as e:
...
def start_recording():
    app.logger.info("Starting Recording process")
    ...
    recording_status = {
        "recording": True,
        "current_file": filename  # ✅ Store filename
    }
    with open('/home/pi/webapp/recording_status.json', 'w') as f:
        json.dump(recording_status, f)

    app.logger.info(f"Recording started: {filename}")

    # Use GStreamer to record audio and save it to a file (MP3 format)
...
def stop_recording():
    ...
    if ffmpeg_process:
        ...
        # ✅ Clear recording status in JSON so the latest file can appear
        with open('/home/pi/webapp/recording_status.json', 'w') as f:
            json.dump({"recording": False, "current_file": None}, f)
    ...
    app.logger.info("Recording stopped, status reset.")
    return redirect(url_for('index'))


@app.route('/manage_recordings')
def manage_recordings():
    app.logger.info("User accessed recordings page")

    # Initialize recording_file to None
    recording_file = None

    # Load recording status
    try:
        with open('/home/pi/webapp/recording_status.json', 'r') as f:
            status = json.load(f)
            if status.get("recording"):
                recording_file = status.get("current_file")
    except (FileNotFoundError, json.JSONDecodeError):
        recording_file = None  # Default if the file is missing or corrupt

    app.logger.info(f"Currently Recording File: {recording_file}")

    # Get all MP3 files in the recordings directory
    mp3_files = [
        os.path.join(RECORDINGS_DIR, f) for f in os.listdir(RECORDINGS_DIR) if f.endswith('.mp3')
    ]
    app.logger.info(f"MP3 Files Before Filtering: {mp3_files}")
    ...
    for file_path in sorted(mp3_files, key=os.path.getmtime, reverse=True):
        ...
        try:
            ...
            app.logger.error(f"Error retrieving metadata for {filename}: {e}")
            file_info = {
                'name': filename,
                'size': round(os.path.getsize(file_path) / 1024, 2),
                'date': datetime.fromtimestamp(os.path.getmtime(file_path)).strftime('%m-%d-%Y %H:%M:%S')
...
@app.route("/audiomonitor", methods=["GET", "POST"])
def audiomonitor():
    app.logger.info("Accessing Audiomonitor")
    global pipeline
    ...
    # ✅ Fix: Ensure devices is extracted properly
    devices = get_audio_devices()

    if isinstance(devices, Response):
        devices = devices.get_json()  # ✅ Convert the Response object to JSON data

    if not isinstance(devices, list):  # ✅ Ensure it's iterable
        print("Error: Expected devices to be a list, but got:", type(devices))
        devices = []  # Default to an empty list to prevent iteration errors
...
def save_metadata():
    app.logger.info("Metadata saved")
    metadata = {
